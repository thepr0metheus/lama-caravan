...
PINS = [
    ...
    ("kanban_names_a_port_no_client_owns",
     ...
     '(m.canvasBoardClients([{ id: "skynet:proxy:8022", port: 8022, label: "proton" }]))',
     '[0,[8022]]',
    ...
    ("caption_when_the_agent_card_cannot_hold_the_controls",
     '',
     '[m.clientNeedsCaption(0), m.clientNeedsCaption(1), m.clientNeedsCaption(2), m.clientNeedsCaption(10)]',
    ...
    ("a_scout_machines_agents_get_cards_too",
     'st.setTopology({ ...st.topology, proxies: PROXIES,'
     ' clients: [{ id: "c1", agentUrl: "http://h:8092", lastSeen: 111, agents: [{ id: "a1" }, { id: "a2" }] }] });',
     'm.clientLaneAgentCards(st.topology.clients[0], []).length',
     '2',
     "positive: у машины со скаутом и без пометки «ручной» агенты — тоже отдельные блоки: каждый клиент заведён руками, "
     "а раскладка HOST/VMS/DOCKER пересказывала слово скаута об агентах, которого больше нет"),
    ...
    ("a_leftover_live_report_is_ignored",
     "st.setTopology({ ...st.topology, assignments: STORED(), proxies: PROXIES, clients: LIVE([{ role: \"primary\", proxyId: \"skynet:proxy:23002\", endpoint: \"http://h:23002/v1\" }]) });",
     "[m.topologyBoardAssignmentsForHost(\"c1\")[0].routes.map((r) => [r.role, r.proxyId]), m.topologyAssignmentsForHost(\"c1\")[0].routes.map((r) => r.proxyId)]",
     "[[[\"primary\",\"skynet:proxy:23001\"]],[\"skynet:proxy:23001\"]]",
     "negative: живой отчёт, оставшийся в записи от старого скаута, ничего не решает — доска и простой читатель берут сохранённое; раньше отчёт побеждал по роли"),
    ("board_draws_the_stored_rows_as_they_are",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", name: \"C\" }], assignments: { c1: { assignments: [{ agentId: \"a1\", routes: [   { role: \"fallback\", proxyId: \"skynet:proxy:23002\", endpoint: \"http://h:23002/v1\" },   { role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\", contextLength: 8192 }] }] } } });",
     "(b => [b.map((r) => r.routes.map((x) => [x.role, x.proxyId, x.contextLength ?? null])), JSON.stringify(b) === JSON.stringify(m.topologyAssignmentsForHost(\"c1\"))])(m.topologyBoardAssignmentsForHost(\"c1\"))",
     "[[[[\"fallback\",\"skynet:proxy:23002\",null],[\"primary\",\"skynet:proxy:23001\",8192]]],true]",
     "positive: доска рисует ровно сохранённые строки — все роли в их порядке, с настройками; один читатель, два имени, второго правила нет"),
    ("stored_only",
    ...
    # ── fold: an agent folded into its line (card-fold.js, card-rows.js) ──
    ("agent_fold_line",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] });",
     "(h => [(h.match(/data-fold-mode=\"([a-z]+)\"/) || [])[1] || (h.trim().startsWith(\"<div class=\\\"topology-agent\") ? \"card\" : \"?\"), (h.match(/data-topology-route-handle/g) || []).length, h.indexOf(\"data-topology-route-handle\") < h.indexOf(\"<div class=\\\"topology-agent \"), h.includes('data-t=\"agent-row\"'), h.includes('<span class=\"ar-kind\">manual</span>'), (h.match(/<span class=\"ar-port\"[^>]*>([^<]*)</) || [])[1]])(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\", modelName: \"hemi-proxy\", contextLength: 256000 }] }])[0].html)",
     "[\"line\",1,true,true,true,\":23001\"]",
     "positive: агент с маршрутом в свёрнутой ленте — строка; ручка каната одна и стоит на строке, карточка за ней без ручки"),
    ("agent_fold_line_shows_only_what_is_set",
     "st.setTopology({ ...st.topology, proxies: [{ ...PROXIES[0], clientTimeoutSeconds: 1800 }, PROXIES[1]], clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] });",
     "(h => { const line = (h.match(/<div class=\"fold-row agent-row\".*?<\\/div><\\/div>/s) || [\"\"])[0]; return [line.includes(\"hemi-proxy\"), />[^<]*—[^<]*</.test(line), line.includes(\"fallback\"), line.includes('<span class=\"ar-meta\">⏳ 1800s</span>'), line.includes('<span class=\"ar-meta\">📏 256000</span>')]; })(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\", modelName: \"hemi-proxy\", contextLength: 256000 }] }])[0].html)",
     "[true,false,false,true,true]",
     "positive: в строке только заданное — модель, ⏳ ожидание с порта и 📏 окно маршрута теми же словами, что чипы; прочерков в тексте и пустого fallback нет"),
    ("agent_fold_two_routes_two_handles",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] });",
     "(h => [(h.match(/data-fold-mode=\"([a-z]+)\"/) || [])[1] || (h.trim().startsWith(\"<div class=\\\"topology-agent\") ? \"card\" : \"?\"), (h.match(/data-topology-route-handle/g) || []).length, h.includes(\">↪ :23002<\")])(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\" }, { role: \"fallback\", proxyId: \"skynet:proxy:23002\", endpoint: \"http://h:23002/v1\" }] }])[0].html)",
     "[\"line\",2,true]",
     "positive: основной и запасной — две строки маршрута, ровно две ручки на всю карточку"),
    ("agent_no_route_stays_full_and_says_so",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] });",
     "(h => [(h.match(/data-fold-mode=\"([a-z]+)\"/) || [])[1] || (h.trim().startsWith(\"<div class=\\\"topology-agent\") ? \"card\" : \"?\"), h.includes('data-t=\"agent-no-route\"'), h.includes(\"No route — this agent goes nowhere\")])(m.clientLaneAgentCards(st.topology.clients[0], [])[0].html)",
     "[\"card\",true,true]",
     "negative: агент без маршрута не сворачивается и говорит вслух, что никуда не ходит (раньше — две тихие пустые рамки)"),
    ("agent_fold_pinned",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] }); cf.CARD_FOLD.pinned.add(\"agent:c1:a1\");",
     "(h => [(h.match(/data-fold-mode=\"([a-z]+)\"/) || [])[1] || (h.trim().startsWith(\"<div class=\\\"topology-agent\") ? \"card\" : \"?\"), (h.match(/data-topology-route-handle/g) || []).length, h.includes(\"agent-row\"), h.includes('data-t=\"fold-unpin\"')])(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\", modelName: \"hemi-proxy\", contextLength: 256000 }] }])[0].html)",
     "[\"pinned\",1,false,true]",
     "positive: закреплённый — полная карточка на месте со своей ручкой, строки нет, есть ▴"),
    ("agent_fold_full_lane",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] }); cf.CARD_FOLD.densities = { clients: \"full\" };",
     "(h => [(h.match(/data-fold-mode=\"([a-z]+)\"/) || [])[1] || (h.trim().startsWith(\"<div class=\\\"topology-agent\") ? \"card\" : \"?\"), (h.match(/data-topology-route-handle/g) || []).length])(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\", modelName: \"hemi-proxy\", contextLength: 256000 }] }])[0].html)",
     "[\"card\",1]",
     "positive: лента клиентов «full» — карточка как раньше"),
    ("agent_fold_cells_lane_does_not_touch_clients",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] }); cf.CARD_FOLD.densities = { cells: \"full\" };",
     "(h => (h.match(/data-fold-mode=\"([a-z]+)\"/) || [])[1] || (h.trim().startsWith(\"<div class=\\\"topology-agent\") ? \"card\" : \"?\"))(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\", modelName: \"hemi-proxy\", contextLength: 256000 }] }])[0].html)",
     "\"line\"",
     "negative: «full» у ячеек не разворачивает клиентов"),
    ("agent_incident_stays_full",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, agents: [{ id: \"a1\", name: \"hotel\" }] }] }); st.ui.latestSystemMonitor = { latest: { correlatedActivity: { byProxy: { p1: { port: 23001, last: { status: \"500\", error: \"boom\", label: \"p1\", startedAt: 1700000000, durationMs: 1000 } } } } } };",
    ...
    # ── no one may call an agent dead; anyone may remove it ──
    ("every_agent_can_be_removed",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, state: \"online\", lastSeen: 1, agents: [{ id: \"a1\", name: \"one\", runtimeDetected: true }, { id: \"a2\", name: \"two\" }] }] });",
     "(cards => cards.map((c) => [c.agentId, /data-t=\"agent-remove\"/.test(c.html) && c.html.includes(`data-agent-delete-id=\"${c.agentId}\"`), c.html.includes(\"agent-stale\")]).sort())(m.clientLaneAgentCards(st.topology.clients[0], [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\" }] }, { agentId: \"a2\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23002\", endpoint: \"http://h:23002/v1\" }] }]))",
     "[[\"a1\",true,false],[\"a2\",true,false]]",
     "positive: у каждого агента свой ✕ — запись оператора удаляется руками; раньше крестик был только у агентов из отчёта скаута"),
    ("no_report_word_changes_a_card",
     "st.setTopology({ ...st.topology, proxies: PROXIES, clients: [{ id: \"c1\", manual: true, state: \"online\", lastSeen: 1, agents: [{ id: \"a1\", name: \"one\", runtimeDetected: true }, { id: \"a2\", name: \"two\" }] }] });",
     "(() => { const routes = [{ agentId: \"a1\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23001\", endpoint: \"http://h:23001/v1\" }] }, { agentId: \"a2\", routes: [{ role: \"primary\", proxyId: \"skynet:proxy:23002\", endpoint: \"http://h:23002/v1\" }] }]; const c = st.topology.clients[0]; const a = m.clientLaneAgentCards(c, routes).map((x) => x.html); const bare = { ...c, state: \"stale\", agents: c.agents.map(({ runtimeDetected, ...x }) => x) }; const b = m.clientLaneAgentCards(bare, routes).map((x) => x.html); return [a.length, JSON.stringify(a) === JSON.stringify(b)]; })()",
     "[2,true]",
     "negative: слово отчёта «запущен / нет» и живость машины ничего не меняют в карточке агента — ни ☠, ни свёртки, ни крестика"),
]
