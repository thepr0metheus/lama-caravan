...
PINS = [
    ...
    ("restore_build_modal_on_a_machine", 'st.setState({ ...st.state, llamaCpp: { version: "version: 9999 (ctl)" } });',
     'await (async () => { m.openRestoreBuildModal("b2", { version: "version: 9900 (def)" }, { hostId: "box-a", name: "Box A", version: "version: 9947 (abc)" }); const a = [F().confirmText.textContent, F().confirmMeta.innerHTML.includes("<strong>b9947 (abc) · Box A</strong>"), F().confirmMeta.innerHTML.includes("<strong>b9900 (def)</strong>"), F().confirmMeta.innerHTML.includes("9999"), F().confirmPath.textContent]; await st.ui.pendingConfirm(); await settle(); return [...a, calls().map((c) => [c.path, c.body]), toastText(), polls()]; })()',
     "[\"Copies the archived llama-server build back into place and checks the clone out at its commit. Running model servers keep their current binary until restarted. If this build misbehaves too: the one you are replacing stays in the machine's archive (restore it back the same way), a build can be made again with ⇪ on the machine's card, and as a last resort the same restore works over ssh, in the scout's folder: bash scripts/update-llama.sh --restore <id>.\", true, true, false, \"b2\", [[\"/api/fleet/llama-restore\", {\"hostId\": \"box-a\", \"id\": \"b2\"}]], \"Restoring the previous build — running cells keep their binary until restarted.\", []]",
     "откат на машине со скаутом: «было» — сборка той машины с её именем (не контроллера), свои пути спасения; подтверждение — POST её скауту через контроллер, тост; опроса обновления контроллера нет — ход сборки показывает карточка машины"),
    ("restore_build_modal_on_a_machine_refused", 'globalThis.__fetchReply["/api/fleet/llama-restore"] = { __status: 502, error: "scout at 10.0.0.5 did not answer" };',
     'await (async () => { m.openRestoreBuildModal("b2", null, { hostId: "box-a", name: "Box A", version: "" }); const from = F().confirmMeta.innerHTML.includes("<strong>? · Box A</strong>"); await st.ui.pendingConfirm(); await settle(); return [from, calls().length, toastText()]; })()',
