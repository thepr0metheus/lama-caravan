...
const PROXIES = () => [
  { id: "skynet:proxy:23001", port: 23001, label: "hotel primary", clientId: "box-a", upstreamHost: "127.0.0.1", upstreamPort: 22001 },
  { id: "skynet:proxy:23002", port: 23002, label: "hotel fallback", clientId: "box-a" },
  { id: "skynet:proxy:23101", port: 23101, label: "crane", clientId: "box-b" },
  { id: "skynet:proxy:8022", port: 8022, label: "proton", clientId: "" },
];
...
const TOPO = (extra = {}) => ({ proxies: PROXIES(), clients: [{ id: "box-a", name: "Box A", agents: [{ id: "hotel" }] }, { id: "box-b", name: "Box B", agents: [{ id: "crane" }] }], routers: [ROUTER()], assignments: {
  "box-a": { assignments: [{ agentId: "hotel", routes: [{ role: "primary", proxyId: "skynet:proxy:23001" }, { role: "fallback", proxyId: "skynet:proxy:23002" }] }] },
...
const reset = () => { st.setTopology(TOPO()); st.ui.topologyCanvasRouterId = "router:a"; st.ui.topologyRouterNodeCfgId = ""; st.ui.pendingConfirm = null; st.ui.latestSystemMonitor = null; localStorage.clear();
  ...
  globalThis.__stubReturns = { "routers.routerById": (rs, id) => rs.find((r) => r.id === id), "routers.saveRouters": async (mut) => { const copy = JSON.parse(JSON.stringify(st.topology.routers)); mut(copy); globalThis.__calls.push(["saveRouters", copy[0]]); }, "routers.topologyRouterOutputLabel": (o) => o.label || o.id, "routers.renderServersBlockHtml": () => "<div class=\"srv\">SERVERS</div>",
    "topology-proxies.canvasBoardClients": boardClients, "topology-proxies._cvProxyIsTombstoned": () => false, "topology-proxies.topologyMutedProxyIds": () => new Set(), "topology-proxies.topologyProxyOwner": (id) => (id === "skynet:proxy:23001" ? { title: "Hotel" } : null), "topology-proxies.editTopologyProxy": rec("editTopologyProxy"),
...
PINS = [
    # ── helpers ──
    ("agent_group_and_primary_port", '', '[m._cvAgentGroup({ label: "hotel primary" }), m._cvAgentGroup({ label: "hotel  Fallback" }), m._cvAgentGroup({}), m._cvPrimaryPort({ port: 23002 }), m._cvPrimaryPort({ port: 23001 }), m._cvPrimaryPort({ port: "0" }), m._cvPrimaryPort({})]', '["hotel","hotel","",23001,23001,0,0]',
     "имя группы — подпись без суффикса роли; primary пары — нечётный порт (чётный → порт−1)"),
    ("client_key_and_name", '', '[m.canvasClientKey(PROXIES()[1]), m.canvasClientKey(PROXIES()[3]), m.canvasClientName(PROXIES()[0]), m.canvasClientName(PROXIES()[1]), m.canvasClientName(PROXIES()[2]), m.canvasClientName(PROXIES()[3])]', '["box-a::23001","::8021","hotel","hotel","crane","proton"]',
    ...
    ("client_name_qualified_by_host_when_duplicated", 'st.topology.assignments["box-b"].assignments[0].agentId = "hotel";', '[m.canvasClientName(PROXIES()[0]), m.canvasClientName(PROXIES()[2])]', '["hotel · box-a","hotel · box-b"]', "одноимённые агенты на двух хостах квалифицируются хостом"),
    ...
    ("port_display_name_and_hist_route", '', '[m._portDisplayName(23001), m._portDisplayName(23001, { short: true }), m._portDisplayName(23001, { nameOnly: true }), m._portDisplayName(23101), m._portDisplayName(0), m._cvQHistRoute({ item: { port: 23001 } }), m._cvQHistRoute({ route: "box · hotel fallback" }), m._cvQHistRoute({})]',
     '["Hotel :23001","Hotel:01","Hotel",":23101",":????","Hotel:01","hotel",":?"]', "имя порта: владелец с портом (коротко — две цифры), без владельца — порт; маршрут истории — порт, иначе подпись без роли и хоста"),
    ...
    ("inputs_block_rows", '', '(() => { const h = node("inputs:block").html; return [(h.match(/cv-inputs-client /g) || []).length, h.includes(\'data-cv-in-row="in:skynet:proxy:23001"\'), h.includes(\'data-cv-in-row="in:skynet:proxy:23002"\'), h.includes("cv-in-row primary"), h.includes("cv-in-row fallback unwired"), h.includes("↳"), h.includes("wait 1800s") || h.includes("1800"), h.includes("kanban-unclaimed"), h.includes("data-cv-reconcile"), h.includes("cv-orphan-agents")]; })()',
     '[2,true,true,true,true,true,true,false,false,false]', "блок входов: строка на агента с P и F (фолбэк без ребра — «↳ follows»), бюджет ожидания, непринадлежащих нет; кнопки сверки с живыми отчётами и полосы мёртвых агентов больше нет — их знание пришло от скаута"),
    ...
    ("queue_live_html_empty_and_busy", '', '(() => { const r = ROUTER(); const empty = m.queueNodeLiveHtml(r, r.graph.nodes[0]); globalThis.__overview = { running: [{ group: "127.0.0.1:22001", port: 23001, startedAt: 1 }], queued: [{ group: "127.0.0.1:22001", port: 23101, queue: { position: 1, queuedMs: 20000, timeoutSec: 100 } }] }; const busy = m.queueNodeLiveHtml(r, r.graph.nodes[0]); return [empty.includes("cv-q-empty"), empty.includes("cv-q-qd idle"), busy.includes("cv-q-now-row running"), busy.includes("Hotel :23001"), busy.includes("→ main"), busy.includes("⏳ 1"), busy.includes("cv-q-wait-row"), busy.includes("cv-q-bar-mk spill"), busy.includes("width:20%")]; })()',
