...
PINS = [
    ...
    ('forget_host_asks_then_deletes',
     ...
     'await (async () => { await rc.forgetTopologyHost("h2"); return { calls: calls(), toast: toastText(), confirm: globalThis.__msg }; })()',
     json.dumps({"calls": [{"path": "/api/topology/host/delete", "method": "POST", "body": "{\"hostId\":\"h2\"}"}],
                 "toast": en("hostForgotten", name="Box B"),
                 "confirm": {"msg": en("dlgForgetHost", name="Box B", ago=en("nodeScoutLastReport", ago="10m")),
                             "opts": {"confirmLabel": en("nodeForgetHost")}}}, ensure_ascii=False),
     'positive: диалог называет машину и сколько молчит её скаут; на провод только {hostId}; тост — что машина забыта'),
    ('forget_host_declined',
     'globalThis.__stubReturns["dialogs.appConfirm"] = async () => false; st.topology.hosts = [{ id: "h2", name: "Box B", state: "stale", ageSeconds: 600 }];',
     'await (async () => { await rc.forgetTopologyHost("h2"); return { calls: calls(), toast: toastText() }; })()',
     '{"calls": [], "toast": ""}',
     'negative: отказ в подтверждении — ничего не уходит'),
    ('forget_host_refused_by_the_server',
     'globalThis.__fetchReply["/api/topology/host/delete"] = { __status: 409, error: "host h2 is online: its scout is still reporting; stop the scout first" };',
     'await (async () => { await rc.forgetTopologyHost("h2"); return toastText(); })()',
     '"Error: host h2 is online: its scout is still reporting; stop the scout first"',
     'negative: сервер отказал (скаут ожил) — отказ доходит до оператора, тоста «забыта» нет'),
    ('forget_unknown_host_names_its_id',
     'globalThis.__msg = null; globalThis.__stubReturns["dialogs.appConfirm"] = async (msg) => { globalThis.__msg = msg; return false; };',
     'await (async () => { await rc.forgetTopologyHost("h9"); return globalThis.__msg; })()',
     json.dumps(en("dlgForgetHost", name="h9", ago=en("nodeScoutNeverReported")), ensure_ascii=False),
     'boundary: записи нет — диалог называет id и говорит «не отвечал», а не «0s»'),
