...
| caravan-scout | 2.4.0 on the Linux host (2026-09-24) — installed with `./install.sh` and added from the board (Model servers → ＋ Add scout); knows its machine only, touches only the processes it started and the files it downloaded, reads a model in place where it has the controller's file, and starts its autostart cells when the machine boots |
