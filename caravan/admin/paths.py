...
DEFAULT_MODELS_DIR = Path(os.environ.get("LLAMA_MODELS_DIR", str(LLAMA_HOME / "models"))).expanduser()
...
SERVER_CELLS_DIR = Path(os.environ.get("LAMA_CARAVAN_SERVER_CELLS_DIR",
    str(PROJECT_ROOT / "var/server-cells"))).expanduser()
...
SERVER_BACKUPS_DIR = Path(os.environ.get("LAMA_CARAVAN_SERVER_BACKUPS_DIR",
    str(PROJECT_ROOT / "var/server-backups"))).expanduser()

ADMIN_STATE_FILE = Path(os.environ.get("LLAMA_ADMIN_STATE",
    str(Path.home() / ".local/state/llamacpp-easy-admin/admin.json")))
MONITOR_HISTORY_FILE = Path(os.environ.get("LLAMA_MONITOR_HISTORY",
    str(Path.home() / ".local/state/llamacpp-easy-admin/monitor-history.json")))
INCIDENT_LOG_FILE = Path(os.environ.get("LLAMA_INCIDENT_LOG",
    str(Path.home() / ".local/state/llamacpp-easy-admin/incident-log.jsonl")))
...
_BENCH_CACHE_DIR = PROJECT_ROOT / ".bench_cache"
...
OPENCLAW_CONFIG_CACHE_FILE = Path(os.environ.get("OPENCLAW_CONFIG_CACHE_FILE",
    str(Path.home() / ".config" / "llamacpp-easy-admin" / "openclaw-config-cache.json")))
