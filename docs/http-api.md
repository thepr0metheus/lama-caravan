...
| `POST /api/engines/load` · `POST /api/engines/unload` | `{hostId, kind, model, contextLength?}` — load a model into an engine next to a machine's cells, or unload it, through its scout (2.14+). The scout answers at once and acts on its own thread; the engines it answers with go into the host record, so the model shows as loading on the next read. Refused before the scout for an unknown action, a machine with no report, an engine the machine does not report; the scout's own refusal comes back in its words. Answers `{ok, hostId, kind, model, op, topology}`. |
