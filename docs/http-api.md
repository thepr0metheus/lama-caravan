...
| `POST /api/engines/load` · `POST /api/engines/unload` · `POST /api/engines/delete` | `{hostId, kind, model, contextLength?, hold?, force?}` — (`delete`, scout 2.17+: an Ollama model's files removed; not a loaded one) — load a model into an engine next to a machine's cells, or unload it, through its scout (2.14+). The scout answers at once and acts on its own thread; the engines it answers with go into the host record, so the model shows as loading on the next read. Refused before the scout for an unknown action, a machine with no report, an engine the machine does not report; the scout's own refusal comes back in its words. Answers `{ok, hostId, kind, model, op, topology}`. `hold` (scout 2.15+): seconds the model stays after its last use, or `-1` — until unloaded; only where the engine's `holds` says it can be told. A load that would not fit into the cards' free memory is not started (scout 2.15+): the answer is `{ok: false, …, short: {needBytes, freeBytes, basis}}` with status 200, and the same load with `force: true` (a JSON true only) starts anyway. |
