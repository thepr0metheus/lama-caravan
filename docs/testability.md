...
Generated from the source, not from memory — 362 values. Regenerate with
...
**node** — `node-disconnect`, `node-poweroff`, `node-power-schedule`, `node-reboot`, `node-scout-old`, `node-scout-silent`
